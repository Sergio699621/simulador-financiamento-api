...
def simular_financiamento():
    try:
        ...
        # Validação dos dados de entrada com valores padrão
        valor_imovel = dados.get('valorImovel', 0)
        valor_entrada = dados.get('valorEntrada', 0)
        prazo_anos = dados.get('prazoAnos', 30) # Padrão: 30 anos
        taxa_juros_anual = dados.get('taxaJuros', 11.5) # Padrão: 11.5%
        sistema_amortizacao = dados.get('sistemaAmortizacao', 'sac')
        
        if valor_imovel <= 0 or prazo_anos <= 0 or taxa_juros_anual <= 0:
            return jsonify({'error': 'Dados inválidos. Valor do imóvel, prazo e taxa de juros devem ser maiores que zero.'}), 400
